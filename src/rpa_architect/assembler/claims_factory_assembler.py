...
def _build_project_json(project_name: str, main_entry: str) -> str:
    ...
    payload = {
        "projectVersion": "1.0.13",
        "description": f"{project_name} — generated by autonomous-rpa-architect v0.6",
        "name": project_name,
        # The `main` field MUST point to the CodedWorkflow .cs file —
        # NOT Main.xaml. If main points to the empty-Sequence Main.xaml,
        # the runtime executes that (does nothing) and exits without ever
        # invoking the compiled CodedWorkflow. In v0.5 this was handled
        # by InvokeWorkflowFile inside Main.xaml, but that causes NullRef
        # in multi-namespace projects (BW-18). Pointing main directly at
        # the .cs file works because UiPath resolves it to the compiled
        # [Workflow] Execute() method in the DLL.
        "main": f"{main_entry}.cs",
        "dependencies": {
        ...
        # entryPoints: In v0.5, "Main.xaml" worked because the XAML
        # contained an InvokeWorkflowFile pointing at the .cs. For v0.6,
        # using the .cs file directly as the entry point works better
        # with multi-namespace projects. The UiPath runtime resolves the
        # [Workflow] Execute() method from the compiled DLL.
        "entryPoints": [
            {
                "filePath": f"{main_entry}.cs",
                "uniqueId": str(
...
def assemble_claims_factory(
    ...
    for process_name, (main_class, cs_file, specific_fn) in specs.items():
        ...
        # Main.xaml — kept as a minimal empty sequence. Required by BW-12
        # (the `main` field is needed), but the real entry point is the
        # CodedWorkflow .cs file referenced in entryPoints. The
        # InvokeWorkflowFile approach from v0.5 doesn't work cleanly
        # with multi-namespace projects (BW-18 — NullRef in
        # WorkflowLoader.GetWorkflowActivity).
        bare_main = (
            '<?xml version="1.0" encoding="utf-8"?>\n'
        ...
        (project_dir / "Main.xaml").write_text(bare_main, encoding="utf-8")
        # main_class = project name (e.g. "ClaimsDispatcher")
        # cs_file = the actual workflow .cs file (e.g. "DispatcherMain.cs")
        # The main_entry param of _build_project_json controls what file
        # the runtime loads — must be the .cs file WITHOUT extension
        # because _build_project_json appends ".cs".
        workflow_name = cs_file.replace(".cs", "")  # "DispatcherMain"
        (project_dir / "project.json").write_text(
            _build_project_json(
                main_class,     # project name
                workflow_name,  # actual workflow file stem
            ),
            encoding="utf-8",
        )
